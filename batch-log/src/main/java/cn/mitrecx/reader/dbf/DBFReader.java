			...
			for( int i=0; i<this.header.fieldArray.length; i++) {
				...
				switch( this.header.fieldArray[i].getDataType()) {
					...
					case 'D':
						...
						byte b_date[] = new byte[ this.header.fieldArray[i].getFieldLength()];
						dataInputStream.read( b_date);
						recordObjects[i] = new String( b_date, characterSetName);
						break;
...
//						break;
						byte t_float[] = new byte[ this.header.fieldArray[i].getFieldLength()];
						dataInputStream.read( t_float);
						recordObjects[i] = new String( t_float, characterSetName);
						break;
...
//							
//							//added by world.chen for purpose when -.--- happed then convert to 0
...
//						break;
						...
						byte t_numeric[] = new byte[ this.header.fieldArray[i].getFieldLength()];
						dataInputStream.read( t_numeric);
						recordObjects[i] = new String( t_numeric, characterSetName);
						break;
...
//						break;
						...
						byte t_logical[] = new byte[ this.header.fieldArray[i].getFieldLength()];
						dataInputStream.read( t_logical);
						recordObjects[i] = new String( t_logical, characterSetName);
						break;
...
//						break;
						byte t_m[] = new byte[ this.header.fieldArray[i].getFieldLength()];
						dataInputStream.read( t_m);
						recordObjects[i] = new String( t_m, characterSetName);
						break;
				...
				switch( this.header.fieldArray[i].getDataType()) {
					...
					case 'D':
						...
						try {
							...
							//yyyymmdd
							recordObjects[i] = sdf.format(calendar.getTime());
					...
					case 'N':
						...
						try {
							...
							if( t_numeric.length > 0 && !Utils.contains( t_numeric, (byte)'?')) {
								...
								recordObjects[i] = new String( t_numeric);
							}
							else {
